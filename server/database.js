...
class Database {
  ...
  /**
   * Récupération des usernames, password et salt
   */
  getCredentials () {
    return this.executeQuery('SELECT * FROM Credentials')
  }

  /**
   * Mise à jour du hashedPassword et salt
   * @param {Array} cred : Tableau contenant le hashedPassword, le salt et le username 
   */
  setCredentials (cred) {
    return this.executeQuery('UPDATE Credentials SET hashedPassword = ? , salt = ? WHERE username = ?', cred)
  }
  ...
  async addOrder (datas) {
    ...
    const res = await this.executeQuery(insertOrder, [datas.lastName, datas.firstName, datas.phone, datas.totalPrice, datas.timeSlot])
